...
import com.example.task_manager.integration.AbstractIntegrationTest;
...
@SpringBootTest
@Transactional
public class TaskRepositoryContainerTest extends AbstractIntegrationTest {
    ...
    @Test
    public void shouldGroupTasksPerHourWithTimeZone_Postgres() {

        // given — UTC timestamps (как в реальной системе)
        Task t1 = new Task();
        t1.setTitle("t1");
        ...
        // then — ищем час, который соответствует Tokyo offset
        Object[] target = result.stream()
                .filter(r -> {

                    LocalDateTime hour = ((Timestamp) r[0])
                            .toLocalDateTime();
        ...
        long count = ((Number) target[1]).longValue();
